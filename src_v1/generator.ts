...
export class Generator {
    ...
    switch (node.kind) {
      ...
      case ts.SyntaxKind.CallSignature: {
        if (ts.isCallSignatureDeclaration(node)) {
          return this.readCallSignatureDeclaration(
            node,
            sourceFile,
            globalPrefix,
          );
      ...
      case ts.SyntaxKind.ConstructSignature: {
        ...
        if (ts.isConstructSignatureDeclaration(node) && globalPrefix) {
          return this.readConstructSignatureDeclaration(
            node,
            sourceFile,
            globalPrefix,
          );
      ...
      case ts.SyntaxKind.IndexSignature: {
        if (ts.isIndexSignatureDeclaration(node)) {
          return this.readIndexSignatureDeclaration(
            node,
            sourceFile,
            globalPrefix,
          );
  ...
  public visitType(typeNode: ts.Node, sourceFile: ts.SourceFile): Ast {
    ...
    switch (typeNode.kind) {
      ...
      default: {
        ...
        if (typeNode.kind >= 83 && typeNode.kind <= 165) {
          parameter.setKind(typeNode.kind).setText(
    ...
    if (parameter.getKind() === undefined) {
      console.error(
        `visitType: Unhandled kind '${ts.SyntaxKind[typeNode.kind]}'`,
      );
    }
  ...
  public readModuleDeclaration(
    ...
    const ast = new Ast().setId(formatId(name, globalPrefix)).setName(
   ...
   */
  public readIndexSignatureDeclaration(
    node: ts.IndexSignatureDeclaration,
    sourceFile: ts.SourceFile,
    _globalPrefix = "",
  ): Ast {
    const ast = new Ast().setKind(node.kind).addType(
      this.visitType(node.type, sourceFile),
    );
   ...
   */
  public readInterfaceDeclaration(
    ...
    const memberPrefix = this.getPrefixForInterfaceMembers(
      node,
      sourceFile,
      "",
    );
    ...
    let hasObjectHeritage = interfaceName === "Object" ||
      interfaceName === "ObjectConstructor";

    if (node.heritageClauses) {
      for (const heritageClause of node.heritageClauses) {
        const heritageAst = this.visitStatements(
          heritageClause,
          sourceFile,
          ast.getName(),
        );
      ...
      if (cachedObject) {
        for (const [fileName, statementNode] of cachedObject) {
          ...
          if (statementSourceFile) {
            const statement = this.visitStatements(
              statementNode,
              statementSourceFile,
              ast.getName(),
            );
  ...
  public readHeritage(
    ...
    const ast = new Ast().setId(globalPrefix).setKind(node.kind).setName(
      globalPrefix,
    );
   ...
   */
  public readCallSignatureDeclaration(
   ...
   */
  public readConstructSignatureDeclaration(
    ...
    if (!globalPrefix) {
      throw new TypeError(
        "generateConstructSignatureDeclaration requires a globalPrefix",
      );
    }
   ...
   */
  public readVariableDeclaration(
  ...
  public readVariableStatement(
    ...
    node.declarationList.declarations.forEach((declaration) => {
      const declarationAst = this.readVariableDeclaration(
        declaration,
        sourceFile,
        globalPrefix,
      );
   ...
   */
  public readTypeAliasDeclaration(
  ...
  public getParameters(
    nodeParameters: ts.NodeArray<ts.ParameterDeclaration>,
    sourceFile: ts.SourceFile,
  ): Ast[] {
   ...
   */
  public getParameter(
    nodeParameter: ts.ParameterDeclaration,
    sourceFile: ts.SourceFile,
  ) {
    const parameter = new Ast()
   ...
   */
  public getPrefixForInterfaceMembers(
    node: ts.InterfaceDeclaration,
    sourceFile: ts.SourceFile,
    _globalPrefix = "",
  ): string {
    const interfaceName = node.name.getText(sourceFile);
    const hasConstructSignatureDeclaration = this.cache.constructors.has(
      interfaceName,
    );
    const isInterfaceNameADeclaredVariable = this.cache.variableNameToTypeMap
      .has(interfaceName);
    const interfaceTypeForDeclaredVariable = this.cache.variableNameToTypeMap
      .get(interfaceName);
    ...
    if (hasConstructSignatureDeclaration) {
      ...
      isInterfaceNameADeclaredVariable &&
      interfaceTypeForDeclaredVariable === interfaceName
    ) {
   ...
   */
  public hasConstructSignatureDeclaration(
