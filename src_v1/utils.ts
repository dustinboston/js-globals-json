...
export function hasDeclarationWithType(
  node: ts.Node,
): node is DeclarationWithType {
  return (ts.isVariableDeclaration(node) || ts.isFunctionDeclaration(node)) &&
    node.type !== undefined &&
    (ts.isTypeReferenceNode(node.type) || ts.isToken(node.type)) &&
    node.name !== undefined;
}
