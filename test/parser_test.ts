...
function createMock<T extends ts.Node>(
  ...
  parser.parse(node, sourceFile);

  return { parser, sourceFile, cache, node, testSpy };
}

Deno.test("Parser.parse", async (t) => {
  await t.step("handles function declaration", () => {
    const { testSpy, node, sourceFile } = createMock(
      `function myFunction(): string;`,
      "readFunctionDeclaration",
    );
    assertSpyCall(testSpy, 0, {
      args: [node as ts.FunctionDeclaration, sourceFile, ""],
    });
  });
  ...
  await t.step("handles type alias declaration", () => {
    const { testSpy, node, sourceFile } = createMock(
      ...
      "readTypeAliasDeclaration",
    );
    assertSpyCall(testSpy, 0, {
      args: [node as ts.TypeAliasDeclaration, sourceFile, ""],
    });
  });

  await t.step("handles variable declaration", () => {
    const { testSpy, node, sourceFile } = createMock<ts.VariableStatement>(
      `var myVariable: string;`,
      "readVariableDeclaration",
    );
    assertSpyCall(testSpy, 0, {
      args: [node.declarationList.declarations[0], sourceFile, ""],
    });
  });

  await t.step("handles module declaration", () => {
    const { testSpy, node, sourceFile } = createMock<ts.ModuleDeclaration>(
      `module MyModule {};`,
  ...
  await t.step("handles interface declaration", () => {
    const { testSpy, node, sourceFile } = createMock<ts.InterfaceDeclaration>(
      `interface MyInterface {};`,
      "readInterfaceDeclaration",
    );
    assertSpyCall(testSpy, 0, { args: [node, sourceFile, ""] });
  });
  ...
  await t.step("handles call signature declaration", () => {
    ...
    const { testSpy, sourceFile } = createMock<ts.CallSignatureDeclaration>(
      code,
      "readCallSignatureDeclaration",
      node,
    );
    assertSpyCall(testSpy, 0, { args: [node, sourceFile, ""] });
  });
  ...
  await t.step(
    "handles construct signature declaration with global prefix",
    () => {
      const generator = createMockParser();
      const mockNode = ts.factory.createConstructSignature(_, [], _);
      const mockSourceFile = {} as ts.SourceFile;
      const readConstructSignatureDeclarationSpy = spy(
        generator,
        "readConstructSignatureDeclaration",
      );
  ...
  await t.step(
    "skips construct signature declaration without global prefix",
    () => {
      const generator = createMockParser();
      ...
      const result = generator.visitStatements(
        mockNode as ts.Node,
        mockSourceFile,
      );
