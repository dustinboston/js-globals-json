...
Deno.test("[cache.visitModuleDeclaration] caches a variable declaration and a statement", () => {
  const cache = createMockCache(
    `declare module MyModule { interface MyInterface {} }`,
  );
  ...
  cache.cacheDeclarations(node, sourceFile, "");

  assert(
    cache.variableNameToTypeMap.size === 1,
    "There should be one item in the variable declarations cache.",
  );
  assert(
    cache.variableNameToTypeMap.get("MyModule") === "MyModule",
    '"MyModule" should be defined in the variable declarations cache.',
  );
  assert(
    cache.statementsCache.size === 2,
    "There should be two items in the statements cache.",
  );

  const myModule = cache.statementsCache.get("MyModule");
  assert(
    myModule && ts.isModuleDeclaration(myModule[0][1]),
    "The first item in the statements cache should be a module declaration",
  );

  const myInterface = cache.statementsCache.get("MyModule.MyInterface");
  assert(
    myInterface && ts.isInterfaceDeclaration(myInterface[0][1]),
    "The second item in the statements cache should be an interface declaration",
  );
});
...
Deno.test("[cache.visitFunctionDeclaration] caches a statement", () => {
  ...
  assert(
    cache.statementsCache.size === 1,
    "There should be one item in the statements cache.",
  );

  const myModule = cache.statementsCache.get("MyInterface");
  assert(
    myModule && ts.isInterfaceDeclaration(myModule[0][1]),
    "The first item in the statements cache should be an interface declaration",
  );
});
...
Deno.test("[cache.visitInterfaceDeclaration] caches a statement and a constructor if present", () => {
  ...
  assert(
    cache.constructors.size === 1,
    "There should be one item in the constructor cache.",
  );
  assert(
    cache.constructors.has("MyInterface"),
    "There should be one item in the constructor cache.",
  );
});
...
Deno.test("[cache.visitVariableDeclaration] caches a variable declaration", () => {
  ...
  cache.cacheDeclarations(node.declarationList.declarations[0], sourceFile, "");
  ...
  assert(
    cache.variableNameToTypeMap.get("myVariable") === "number",
    '"myVariable" should be defined in the variable declarations cache.',
  );
});
...
Deno.test("Caches should collect data from the program", () => {
  ...
  assert(
    cache.variableNameToTypeMap.get("Frobnicator") === "FrobnicatorConstructor",
  );

  assert(cache.variableTypeToNameMap.size === 1);
  assert(
    cache.variableTypeToNameMap.get("FrobnicatorConstructor") === "Frobnicator",
  );
